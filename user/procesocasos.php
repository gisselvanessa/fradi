...
// $consulta = "SELECT mcodigoperson, midformulariogeneral, mnombresComp, mfregistro, mcreador,mstatusat,mobservacionpc FROM mlistareportes WHERE ";

// if (!empty($busqueda)) {
//   $consulta .= "(midformulariogeneral LIKE '%$busqueda%' OR mnombresComp LIKE '%$busqueda%') AND mstatusat ='A'";
// } else {
//   $consulta .= "mstatusat ='A'";
// }
$busqueda = isset($_GET['busqueda']) ? $_GET['busqueda'] : '';

// Construir la consulta SQL con el filtro de búsqueda
$consulta = "SELECT m.mcodigoperson, m.midformulariogeneral, m.mnombresComp,m.mstatusat, m.mfregistro, m.mcreador,m.mobservacionpc, t1.midentificacion AS nombre_identificacion
FROM mlistareportes AS m
LEFT JOIN mdatospersona AS t1 ON m.mcodigoperson = t1.mcodigoperson
WHERE ";

if (!empty($busqueda)) {
  $consulta .= "(m.midformulariogeneral LIKE '%$busqueda%' OR m.mnombresComp LIKE '%$busqueda%' OR t1.midentificacion LIKE '%$busqueda%') AND m.mstatusat = 'A'";
} else {
  $consulta .= "m.mstatusat = 'A'";
}
...
<form action="procesocasos.php" method="GET" class="mb-3" onsubmit="limpiarEspacios()">
  <div class="cntr">
    <div class="col-md-5">
    ...
    <?php foreach ($elementosPagina as $elemento): ?>
      ...
      <td><?php echo $elemento['mfregistro']; ?></td>
        ...
        <td class="text-center">
            ...
            if ($elemento['mstatusat'] === 'A') {
                // Si el estado es 'D', mostramos el botón para abrir el modal con los detalles del trámite
                echo '<button class="btn btn-outline-info" onclick="mostrarDetalleTramite(\'A\', \'' . $elemento['midformulariogeneral'] . '\', \'' . $elemento['mobservacionpc'] . '\')">Caso Aceptado</button>';
            } else {
        ...
        <form id="notificarForm">
          ...
          <div class="form-group">
            ...
            <select class="form-select" id="estadoSelect" name="estado">
              <option value="T">En tr&aacute;mite</option>
          ...
          <div class="form-group">
            <label for="fechaInputSol">Fecha aproximada de soluci&oacute;n:</label>
            <input type="date" class="form-control" id="fechaInputSol" name="fechaInputSol">
          </div>
        </form>
      </div>
      <div class="modal-footer">
        <button type="button" class="btn btn-secondary" data-dismiss="modal">Cancelar</button>
        <button type="button" class="btn btn-primary" onclick="guardarNotificacionTramite()">Guardar</button>
      </div>
    </div>
  </div>
</div>
...
<div class="d-flex justify-content-center">
  <ul class="pagination">
    <?php if ($paginaActual > 1): ?>
      <li class="page-item">
        <a class="page-link" style="background-color: #17a2b8; color: white;" href="procesocasos.php?busqueda=<?php echo $busqueda; ?>&pagina=<?php echo $paginaActual - 1; ?>">Anterior</a>
      </li>
    <?php endif; ?>
    ...
    <?php if ($totalPaginas > 1): ?>
      <?php if ($paginaActual > 3): ?>
        <li class="page-item">
          <a class="page-link" style=" color: #17a2b8;" href="procesocasos.php?busqueda=<?php echo $busqueda; ?>&pagina=1">1</a>
        </li>
      ...
      <?php for ($i = max(1, $paginaActual - 1); $i <= min($paginaActual + 1, $totalPaginas); $i++): ?>
        <li class="page-item <?php echo ($i == $paginaActual) ? 'active' : ''; ?>">
          <a class="page-link" style="color: #17a2b8; background-color: white; " href="procesocasos.php?busqueda=<?php echo $busqueda; ?>&pagina=<?php echo $i; ?>"><?php echo $i; ?></a>
        </li>
      <?php endfor; ?>
      ...
      <?php if ($paginaActual < $totalPaginas - 2): ?>
        ...
        <li class="page-item">
          <a class="page-link" style=" color: #17a2b8;" href="procesocasos.php?busqueda=<?php echo $busqueda; ?>&pagina=<?php echo $totalPaginas; ?>"><?php echo $totalPaginas; ?></a>
        </li>
      <?php endif; ?>
    <?php endif; ?>
    ...
    <?php if ($paginaActual < $totalPaginas): ?>
      <li class="page-item">
        <a class="page-link" style="background-color: #17a2b8; color: white;" href="procesocasos.php?busqueda=<?php echo $busqueda; ?>&pagina=<?php echo $paginaActual + 1; ?>">Siguiente</a>
      </li>
    <?php endif; ?>
  </ul>
</div>
...
function guardarNotificacionTramite() {
  ...
  var fechaSolucion = document.getElementById("fechaInputSol").value;
  ...
  var url = "utils/guardar_notificacion_tramite.php"; // Reemplaza "guardar_notificacion.php" con el archivo que procesa los datos en el servidor.
  var params = "nrodetramite=" + nrodetramite + "&accion=" + accion + "&observaciones=" + observaciones + "&fecha=" + fecha+ "&fechaSol=" + fechaSolucion+ "&user=" + usuario;
  ajax.open("POST", url, true);
  ajax.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
  ajax.send(params);
}
...
function mostrarDetalleTramite(estado, nroTramite, observaciones) {
  ...
  estadoActualInput.value = estado === 'A' ? 'Procesado' : '-';
