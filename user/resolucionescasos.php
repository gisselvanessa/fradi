...
<head>
    ...
    <title>RESOLUCION DE CASOS</title>
...
// $consulta = "SELECT mcodigoperson, midformulariogeneral, mnombresComp, mfregistro, mcreador,mobservacionres, mstatusat FROM mlistareportes WHERE ";

// if (!empty($busqueda)) {
//   $consulta .= "(midformulariogeneral LIKE '%$busqueda%' OR mnombresComp LIKE '%$busqueda%') AND mstatusat ='T'";
// } else {
//   $consulta .= "mstatusat ='T'";
// }
$busqueda = isset($_GET['busqueda']) ? $_GET['busqueda'] : '';

// Construir la consulta SQL con el filtro de búsqueda
$consulta = "SELECT m.mcodigoperson, m.midformulariogeneral, m.mnombresComp,m.mstatusat, m.mfregistro, m.mcreador,m.mobservacionres, t1.midentificacion AS nombre_identificacion
FROM mlistareportes AS m
LEFT JOIN mdatospersona AS t1 ON m.mcodigoperson = t1.mcodigoperson
WHERE ";

if (!empty($busqueda)) {
  $consulta .= "(m.midformulariogeneral LIKE '%$busqueda%' OR m.mnombresComp LIKE '%$busqueda%' OR t1.midentificacion LIKE '%$busqueda%') AND m.mstatusat = 'T'";
} else {
  $consulta .= "m.mstatusat = 'T'";
}
...
<form action="resolucionescasos.php" method="GET" class="mb-3" onsubmit="limpiarEspacios()">
  <div class="cntr">
    <div class="col-md-5">
    ...
    <?php foreach ($elementosPagina as $elemento): ?>
      ...
      <td><?php echo $elemento['mfregistro']; ?></td>
        ...
        <td class="text-center">
            ...
            if ($elemento['mstatusat'] === 'T') {
                // Si el estado es 'D', mostramos el botón para abrir el modal con los detalles del trámite
                echo '<button class="btn btn-outline-info" onclick="mostrarDetalleTramite(\'T\', \'' . $elemento['midformulariogeneral'] . '\', \'' . $elemento['mobservacionres'] . '\')">En trámite</button>';
            } else {
...
<div class="modal fade" id="notificarModal" tabindex="-1" role="dialog" aria-labelledby="notificarModalLabel" aria-hidden="true">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="notificarModalLabel">Noticias VISA - Número de Trámite: <span id="nroTramiteLabel"></span></h5>
        ...
        <form id="notificarForm">
          ...
          <div class="form-group">
            ...
            <select class="form-select" id="estadoSelect" name="estado">
              <option value="F">Resuelto</option>
              <option value="N">Otros-Proceso terminado</option>
            </select>
          </div>
          <div class="form-group">
...
<div class="d-flex justify-content-center">
  <ul class="pagination">
    <?php if ($paginaActual > 1): ?>
      <li class="page-item">
        <a class="page-link" style="background-color: #17a2b8; color: white;" href="resolucionescasos.php?busqueda=<?php echo $busqueda; ?>&pagina=<?php echo $paginaActual - 1; ?>">Anterior</a>
      </li>
    <?php endif; ?>
    ...
    <?php if ($totalPaginas > 1): ?>
      <?php if ($paginaActual > 3): ?>
        <li class="page-item">
          <a class="page-link" style=" color: #17a2b8;" href="resolucionescasos.php?busqueda=<?php echo $busqueda; ?>&pagina=1">1</a>
        </li>
      ...
      <?php for ($i = max(1, $paginaActual - 1); $i <= min($paginaActual + 1, $totalPaginas); $i++): ?>
        <li class="page-item <?php echo ($i == $paginaActual) ? 'active' : ''; ?>">
          <a class="page-link" style="color: #17a2b8; background-color: white; " href="resolucionescasos.php?busqueda=<?php echo $busqueda; ?>&pagina=<?php echo $i; ?>"><?php echo $i; ?></a>
        </li>
      <?php endfor; ?>
      ...
      <?php if ($paginaActual < $totalPaginas - 2): ?>
        ...
        <li class="page-item">
          <a class="page-link" style=" color: #17a2b8;" href="resolucionescasos.php?busqueda=<?php echo $busqueda; ?>&pagina=<?php echo $totalPaginas; ?>"><?php echo $totalPaginas; ?></a>
        </li>
      <?php endif; ?>
    <?php endif; ?>
    ...
    <?php if ($paginaActual < $totalPaginas): ?>
      <li class="page-item">
        <a class="page-link" style="background-color: #17a2b8; color: white;" href="resolucionescasos.php?busqueda=<?php echo $busqueda; ?>&pagina=<?php echo $paginaActual + 1; ?>">Siguiente</a>
      </li>
    <?php endif; ?>
  </ul>
</div>
...
function guardarNotificacion() {
  ...
  var url = "utils/guardar_notificacion_resolucion.php"; // Reemplaza "guardar_notificacion.php" con el archivo que procesa los datos en el servidor.
...
function mostrarDetalleTramite(estado, nroTramite, observaciones) {
  ...
  estadoActualInput.value = estado === 'T' ? 'En trámite' : 'Otros';
